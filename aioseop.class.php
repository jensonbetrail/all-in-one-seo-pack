...
class All_in_One_SEO_Pack {
 	...
 	var $maximum_description_length = 400; // Edited by Djarot Apr 21, 2012
...
*/
	...
	function wp_head() {
		...
		/*echo "\n<!-- All in One SEO Pack $this->version by Michael Torbert of Semper Fi Web Design";*/ // Edited by Djarot Apr 21, 2012
		if ($this->ob_start_detected) {
			echo "ob_start_detected ";
		}
		/*echo "[$this->title_start,$this->title_end] ";
		echo "-->\n";*/ // Edited by Djarot Apr 21, 2012
...
//			$keywords =	$this->get_unique_keywords($keywords);
			...
			if (is_single() || is_page() || $this->is_static_posts_page()) {
				...
				$description = trim(stripcslashes($this->internationalize($aioseop_options['aiosp_home_description'])));
      } elseif (is_attachment()) { //Added to test Edited by Djarot Apr 21, 2012
        //Do nothing to test it. Edited by Edited by Djarot Dec 31, 2011
			} else if (is_category()) {
				/* $description = $this->internationalize(category_description()); */ // Edited by Djarot Apr 21, 2012
			}
			...
			if (isset($description) && (strlen($description) > $this->minimum_description_length) && !(is_home() && is_paged())) {
				...
				if($aioseop_options['aiosp_can'] && is_attachment()){ /*
					...
					$description .= ' ' . $uniqueDesc; */ // Edited by Djarot Apr 21, 2012 - Do this Later
				}
          ...
          //$meta_string .= "\n"; // Edited by Djarot Apr 21, 2012
			...
			if ((is_category() && $aioseop_options['aiosp_category_noindex']) || (!is_category() && is_archive() &&!$is_tag && $aioseop_options['aiosp_archive_noindex']) || ($aioseop_options['aiosp_tags_noindex'] && $is_tag)) {
				if (isset($meta_string)) {
					$meta_string .= ""; // Edited by Djarot Apr 21, 2012
				}
			...
			if ($meta_string != null) {
				echo "$meta_string"; // Edited by Djarot Apr 21, 2012
			}
			...
			if($aioseop_options['aiosp_can']){
				...
				if ($url) {
					...
					echo "".'<link rel="canonical" href="'.$url.'" />'.""; // Edited by Djarot Apr 21, 2012
				}
...
// Thank you, Yoast de Valk, for much of this code.		
	...
	function get_post_description($post) {
		...
		if (!$description) {
			$description = $this->trim_excerpt_without_filters($this->internationalize($post->post_content)); // Edited by Djarot Apr 21, 2012
	...
	function get_original_title() {
		...
		/*} else if (is_search() && isset($s) && !empty($s)) { // Edited by Djarot Apr 21, 2012
			$search = esc_attr(stripcslashes($s));
			$search = $this->capitalize($search);
			$title = $search;*/
		} else if (is_category() && !is_feed()) { /* // Edited by Djarot Apr 21, 2012
			...
			$title = $this->internationalize(wp_title('', false));
		} else if (is_tag()) { /* // Edited by Djarot Apr 21, 2012
			global $utw;
			if ($utw) {
		...
		} else if (is_archive()) { // Edited by Djarot Apr 21, 2012
			/*$title = $this->internationalize(wp_title('', false));*/
		/*} else if (is_404()) { // Edited by Djarot Apr 21, 2012
	...
	function rewrite_title($header) {
		...
		global $wp_query;
		if (!$wp_query) {
			$header .= "<!-- no wp_query found! -->"; // Edited by Djarot Apr 21, 2012
			return $header;	
		}
      ...
      // Edited by Djarot Dec 31, 2011 -> 1. Added ":" instead of nothing, to separate parent & child title. 2. Added "|" before blog name.
...
?>
    ...
    <?php /* by <strong>Michael Torbert</strong> of <strong>Semper Fi Web Design</strong> */ // Edited by Djarot Dec 31, 2011 ?>
    by Michael Torbert of Semper Fi Web Design
<?php /*  // Edited by Djarot Dec 31, 2011 ?>
		<p>
...
<?php if (!$aioseop_options['aiosp_donate']){ /* // Edited by Djarot Apr 21, 2012 ?>
<tr>
<th scope="row" style="text-align:right; vertical-align:top;">
